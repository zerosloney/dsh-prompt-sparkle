 ...
 * 解析不到返回空字符串（不抛错）。
 */
export function resolvePackageVersion(packageName: string, anchorPath?: string): string {
...
function parseVersion(raw: string): ParsedVersion | undefined {
  ...
  if ([major, minor, patch].some((part) => !Number.isInteger(part))) return undefined;
...
function comparePrereleaseIdentifier(left: string, right: string): number {
  ...
  if (leftNumeric) return -1; // 数字标识符 < 字母标识符
...
export function compareVersions(left: string, right: string): -1 | 0 | 1 {
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    if (compared !== 0) return compared as -1 | 0 | 1;
  }
  return 0;
}

/** 版本号是否落在 [lower, upper) 区间（含下界、不含上界）。 */
export function inVersionRange(version: string, lower: string, upper: string): boolean {
  return compareVersions(version, lower) >= 0 && compareVersions(version, upper) < 0;
}

/** 按已知 wire 协议分界把版本归类为兼容家族。 */
export function familyOf(version: string): HarnessCompatFamily {
  if (version === "") return "unknown";
  // 0.1.0-rc.7 及更早（>=0.1.0-rc.7 且 <0.1.0-rc.8）：execute 无 images 参数
  if (inVersionRange(version, "0.1.0-rc.7", "0.1.0-rc.8")) return "legacy-rc7";
  // 0.1.0-rc.8 起（含 0.1.1-rc.x）：execute 带 images 参数
  if (inVersionRange(version, "0.1.0-rc.8", "0.2.0")) return "current";
  return "unknown";
}
 ...
 * 其余包版本一并带回供日志与诊断。
 */
export function detectHarnessVersion(anchorPath?: string): HarnessVersionInfo {
...
/** 人类可读的探测摘要（供启动日志）。 */
export function describeHarness(info: HarnessVersionInfo): string {

...
function apply(ctx) {
  ...
  /* ---- contenteditable composer（dsh >= 0.1.2-rc.1）------------------
   * 0.1.2-rc.1 起 composer 的输入面从 <textarea> 换成了 Lexical 驱动的
   * contenteditable div（[data-composer-input]，data-phase 仍挂其上）。
   * 直接改 DOM 会被 Lexical 的重渲染覆盖，唯一可靠的外部写入原语是
   * focus → 选中全部 → document.execCommand('insertText')：Lexical 会
   * 拦截 beforeinput 并把变更纳入自己的 EditorState，输入机（发送键
   * 解禁/草稿持久化）随之收到通知 —— 已在 0.1.2-rc.1 实机验证，连续
   * 两次全量替换（润色 + 撤销场景）都稳定落地。
   * ------------------------------------------------------------------ */

  /** 递归收集编辑面的纯文本：文本节点直取（\u00a0→空格），<br> 计一个
   * '\n'，块级子元素（P/DIV，Lexical 段落）之间计一个 '\n'——只在块与块
   * 之间加，首块之前不加（否则单段草稿会带上幽灵前导换行，润色/撤销
   * 的读写都会错位）。读取偏移与写回拼接共用同一字符空间。 */
  function collectEditableText(node, out) {
    for (var c = node.firstChild; c !== null; c = c.nextSibling) {
      if (c.nodeType === 3) {
        out.push(String(c.data).replace(/\u00a0/g, ' '))
      } else if (c.nodeName === 'BR') {
        out.push('\n')
      } else if (c.nodeType === 1) {
        if (c.nodeName === 'P' || c.nodeName === 'DIV') {
          var prev = out.length > 0 ? out[out.length - 1] : ''
          if (prev !== '' && prev.slice(-1) !== '\n') out.push('\n')
        }
        collectEditableText(c, out)
      }
    }
  }

  function editableTextOf(el) {
    var parts = []
    collectEditableText(el, parts)
    return parts.join('')
  }

  /** 当前选区在编辑面文本空间中的字符偏移 { start, end }（anchor/focus
   * 顺序无关，start<=end）；选区不在编辑面内时折叠到末尾。 */
  function editableSelectionOffsets(el) {
    var total = editableTextOf(el).length
    var sel = window.getSelection()
    if (sel === null || sel.rangeCount === 0 || sel.isCollapsed) return { start: total, end: total }
    var range = sel.getRangeAt(0)
    if (el.contains(range.startContainer) !== true || el.contains(range.endContainer) !== true) {
      return { start: total, end: total }
    }
    var offsets = { start: -1, end: -1 }
    var walk = function (node, pos) {
      for (var c = node.firstChild; c !== null; c = c.nextSibling) {
        if (c.nodeType === 3) {
          var len = String(c.data).replace(/\u00a0/g, ' ').length
          if (c === range.startContainer) offsets.start = pos + (range.startOffset <= len ? range.startOffset : len)
          if (c === range.endContainer) offsets.end = pos + (range.endOffset <= len ? range.endOffset : len)
          pos += len
        } else if (c.nodeName === 'BR') {
          pos += 1
        } else if (c.nodeType === 1) {
          if ((c.nodeName === 'P' || c.nodeName === 'DIV') && pos > 0) pos += 1
          pos = walk(c, pos)
        }
      }
      return pos
    ...
    walk(el, 0)
    if (offsets.start < 0 || offsets.end < 0) return { start: total, end: total }
    return offsets.start <= offsets.end
      ? { start: offsets.start, end: offsets.end }
      : { start: offsets.end, end: offsets.start }
  }

  /** contenteditable 输入面的忙碌/只读判定：data-phase（与旧 textarea 同
   * 一属性现在挂在 div 上）+ contentEditable 关闭 + aria-disabled。 */
  function editableDisabled(el) {
    var phase = el.getAttribute('data-phase')
    return BUSY_PHASES[phase] === 1
      || el.getAttribute('contenteditable') !== 'true'
      || el.getAttribute('aria-disabled') === 'true'
  }

  /** 当前草稿状态：{ el, text, disabled, selectionStart, selectionEnd, selectedText }；找不到输入框时 disabled。
   * 双路径：优先旧 <textarea>（<= 0.1.1-rc.2），否则找 0.1.2-rc.1+ 的
   * [data-composer-input] contenteditable。 */
  function draftState() {
    var empty = { el: null, text: '', disabled: true, selectionStart: 0, selectionEnd: 0, selectedText: '' }
    ...
    var editable = card.querySelector('[data-composer-input]')
    if (editable === null) return empty
    var text = editableTextOf(editable)
    var offsets = editableSelectionOffsets(editable)
    var editSelected = offsets.end > offsets.start ? text.slice(offsets.start, offsets.end) : ''
    return {
      el: editable,
      text: text,
      disabled: editableDisabled(editable),
      selectionStart: offsets.start,
      selectionEnd: offsets.end,
      selectedText: editSelected,
    }
  }
  ...
  /** 写回草稿。textarea 走原生 setter + input 事件（受控组件路径）；
   * contenteditable（0.1.2-rc.1+ Lexical 输入面）走 **focus → 全选
   * → execCommand('selectAll') → 等一拍让 Lexical 同步内部 selection
   * → 合成 paste 事件**。为何不直接 execCommand('insertText')：Lexical
   * 拦截元素锚定全选时会在 beforeinput 处理器内 normalize 选区为
   * collapsed，insertText 落点漂移（连带旧内容被复制、首尾出现双份
   * 等症状）；合成 paste 走的是 Lexical 自己管理的 paste 路径，对外部
   * 选区在同步一拍后能可靠生效，对带空格的文本也稳定。局部替换在
   * 调用侧拼好整份新草稿后同样全量 paste，selection 偏移仅用于读取
   * 选区片段作为 LLM 输入。paste 之后 Lexical 重新渲染是异步的——
   * 调用方不要立即回读 DOM。 */
  function writeDraft(el, text, selRange) {
    if (el !== null && el !== undefined && el.tagName === 'TEXTAREA') {
      var nextText = text
    ...
    if (el !== null && el !== undefined) {
      var full = editableTextOf(el)
      var replacement = text
      if (selRange && typeof selRange.start === 'number' && typeof selRange.end === 'number' && selRange.end >= selRange.start) {
        replacement = full.slice(0, selRange.start) + text + full.slice(selRange.end)
      }
      el.focus({ preventScroll: true })
      document.execCommand('selectAll')
      // 等一拍让 Lexical 的 selectionchange 处理器把全选同步到内部
      // selection，否则 paste 处理器读到的 selection 与 DOM 不一致，
      // 落点会漂到末尾。120ms 在实测中稳定。
      setTimeout(function () {
  ...
  function runSparkle() {
    ...
    function sparkleCall(mode) {
      var request = sparkleRequest(mode, execute, sessionId, cmd)
    ...
    // 版本自适应的命令分发：current 线协议传 images=[]，legacy-rc7 只传两个参数
    var sparkleMode = getExecuteModeSync(ctx)
    if (sparkleMode !== null) {
      sparkleCall(sparkleMode)
    } else {
      probeExecuteMode(ctx, sessionId).then(function (mode) {
        executeModeCache = mode
        sparkleCall(mode)
      }, function () {
        sparkleCall({ images: true, source: 'probe-rejected' })
      })
    }
  }
...
/* ---- deepseek-harness 版本兼容层（commands.execute 线协议自适应） ----
 * dsh 0.1.0-rc.8+（含 0.1.1-rc.x）的 commands.execute 声明了 `images`
 * 业务参数：客户端传 execute(sessionId, line, [])。
 * dsh 0.1.0-rc.7 的 execute 只有 `line` 一个业务参数：第 3 参会被 remote
 * proxy 当作 AbortSignal 并在 AbortSignal.any 处抛 TypeError，润色必败。
 * 因此必须按宿主版本选择调用形态：
 *  - 优先读取 typert registry 的 wire 描述符（两个版本的 typert 实现
 *    完全一致，零网络探测、零副作用）；
 *  - typert 不可用时回退为一次幂等探测（未知命令名不会产生任何会话
 *    副作用：current 返回 {ok:true,value:undefined}，legacy-rc7 在
 *    客户端代理层即失败返回 {ok:false}）。
 */
var executeModeCache = null

function sparkleRequest(mode, execute, sessionId, cmd) {
  return mode.images ? execute(sessionId, cmd, []) : execute(sessionId, cmd)
}

function getExecuteModeSync(ctx) {
  if (executeModeCache !== null) return executeModeCache
  try {
    var typert = ctx && typeof ctx.get === 'function' ? ctx.get('typert') : undefined
    var descriptors = typert && typert.remotes && typeof typert.remotes.list === 'function' ? typert.remotes.list() : undefined
    if (descriptors !== undefined) {
      for (var i = 0; i < descriptors.length; i++) {
        var descriptor = descriptors[i]
        if (descriptor && descriptor.namespace === 'commands' && descriptor.method === 'execute' && Array.isArray(descriptor.parameters)) {
          var hasImages = false
          for (var j = 0; j < descriptor.parameters.length; j++) {
            if (descriptor.parameters[j] && descriptor.parameters[j].name === 'images') { hasImages = true; break }
          }
          executeModeCache = { images: hasImages, source: 'typert-descriptor' }
          return executeModeCache
        }
      }
    }
  } catch (error) {
...
function probeExecuteMode(ctx, sessionId) {
  var remote = ctx && ctx.remote
  var execute = remote && remote.commands && remote.commands.execute
  if (typeof execute !== 'function') return Promise.resolve({ images: true, source: 'default' })
  var probe = execute(sessionId, '/__dsh_prompt_sparkle_probe__', [])
  return Promise.resolve(probe).then(function (result) {
    return { images: result && result.ok === true, source: 'probe' }
  }, function () {
    return { images: false, source: 'probe-rejected' }
  })
}
...
// 测试钩子：仅当宿主显式开启 window.__SPARKLE_TEST__ 时暴露内部实现，
// 供兼容性测试套件在 Node 环境下直接验证线协议判定逻辑；生产环境不生效。
if (typeof window !== 'undefined' && window.__SPARKLE_TEST__) {
  window.__sparkleTest = {
    getExecuteModeSync: getExecuteModeSync,
    sparkleRequest: sparkleRequest,
    probeExecuteMode: probeExecuteMode,
    resetExecuteModeCache: function () { executeModeCache = null },
  }
}
